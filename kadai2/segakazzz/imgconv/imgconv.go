...
func newConverter(dirname string, input string, output string) (*converter, error) {
	var (
		ErrInputExt  = errors.New("Input extension is not valid. Select one from jpg/png")
		ErrOutputExt = errors.New("Output extension is not valid. Select one from jpg/png")
		ErrExtSame   = errors.New("Input and Output extensiton is the same. No convertion is needed")
	)
	...
	default:
		return nil, ErrInputExt
	}
	switch output {
	case "jpg", "png":
		output = strings.ToLower(output)
	default:
		return nil, ErrOutputExt
	}

	if input == output {
		return nil, ErrExtSame
	}
